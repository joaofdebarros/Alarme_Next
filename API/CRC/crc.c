...
// lookup table for CRC8 with 0x8D poly calculation, this is for better performance
const uint8_t CRC8_0x1D_LOOKUP_TABLE[] = {
...
// lookup table for CRC32 calculation, this is for better performance
...
uint8_t crc8(uint8_t *data, uint8_t len) {
  ...
  for (i = 0; i < len; i++)  {
    index = crc ^ data[i]; //calculate the index to search in table
    crc = CRC8_0x1D_LOOKUP_TABLE[index]; // get the crc value for the table
  }

  return crc ^ FINAL_XOR; //do the final xor and return the crc value
}

uint32_t crc32(uint32_t initial, uint8_t *data, uint16_t len){
    int i;
    uint32_t crc32val = initial;
    crc32val ^= 0xFFFFFFFF;

    for (i = 0;  i < len;  i++) {
        crc32val = CRC32_LOOKUP_TABLE[(crc32val ^ data[i]) & 0xFF] ^ ((crc32val >> 8) & 0x00FFFFFF);
    }
